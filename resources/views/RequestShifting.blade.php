...
<main class="min-h-screen relative overflow-hidden">
    ...
    <div class="relative z-10 flex flex-col items-center justify-center min-h-screen px-4 sm:px-6">
        <div class="w-full max-w-sm space-y-6 bg-black/40 rounded-lg p-6">
            <h2 class="text-xl font-semibold text-white text-center">REQUEST SHIFT</h2>
            ...
            <form action="{{ route('request-shifting.store') }}" method="POST" class="space-y-4">
                ...
                <div class="space-y-2">
                    <label for="shift_type" class="block text-white text-sm font-semibold tracking-wide">
                        Tipe Shift
                    </label>
                    <select
                        id="shift_type"
                    ...
                    >
                        ...
                        <option value="WFO" {{ old('shift_type') === 'WFO' ? 'selected' : '' }}>WFO</option>
                        <option value="WFH" {{ old('shift_type') === 'WFH' ? 'selected' : '' }}>WFH</option>
                    </select>
                </div>
